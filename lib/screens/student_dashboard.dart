...
class _StudentDashboardState extends State<StudentDashboard> {
      ...
      title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ShaderMask(
          shaderCallback: (bounds) => const LinearGradient(
              colors: [Colors.purple, Colors.blue]).createShader(bounds),
          child: const Text('Achivo Dashboard',
              style: TextStyle(
          ...
          child: Row(children: [
            Column(
              ...
              children: [
                ...
                Text('Year ${_studentProfile!.displayYear}',
    ...
    return Drawer(
      child: Container(
        ...
        child: Column(children: [
          ...
          // ── NEW: Fee Payment ──────────────────────────────
          _drawerItem(Icons.account_balance_wallet, 'Fee Payment',  6),

          const Spacer(),
          _drawerItem(Icons.logout, 'Sign Out', -1, onTap: _handleSignOut),
          const SizedBox(height: 16),
        ]),
      ),
    );
  ...
  Widget _buildBody() {
    ...
    // ── NEW: Fee Dashboard ─────────────────────────────────
      case 6:
        return StudentFeeDashboard(studentId: _studentProfile!.id);
      default:
        return OverviewPage(
          student: _studentProfile!,
          stats:   _dashboardStats!,
  ...
  Widget _buildWelcomeHeader() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Text("Here's an overview of your academic journey",
      ...
      Row(children: [
        Expanded(
          child: _statCard(
            ...
            subtitle: '${stats.totalCredits == 0 ? 0 : ((stats.creditsCompleted / stats.totalCredits) * 100).toInt()}% completed',
            color:    Colors.teal,
  ...
  Widget _buildQuickActions(BuildContext context) {
    return Container(
      ...
      child: Column(children: [
        ...
        Row(children: [
          Expanded(
            child: _actionCard('Attendance', Icons.how_to_reg, Colors.indigo, () {
              Navigator.push(context, MaterialPageRoute(
                  builder: (_) => StudentAttendancePage(studentId: student.id)));
            }),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: _actionCard('Marks', Icons.grade, Colors.teal, () {
              Navigator.push(context, MaterialPageRoute(
                  builder: (_) => StudentMarksPage(studentId: student.id)));
            }),
          ),
        ]),
        ...
        Row(children: [
          Expanded(
            child: _actionCard('Apply Leave', Icons.event_busy, Colors.purple, () {
              Navigator.push(context, MaterialPageRoute(
                  builder: (_) => LeaveApplicationPage(studentId: student.id)));
            }),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: _actionCard('Upload Documents', Icons.upload_file, Colors.blue, () {
              Navigator.push(context, MaterialPageRoute(
                  builder: (_) => DocumentsPage(studentId: student.id)));
            }),
          ),
        ]),
        const SizedBox(height: 12),
        // ── NEW: Fee Payment quick action ──────────────────────
        Row(children: [
          Expanded(
            child: _actionCard('Fee Payment', Icons.account_balance_wallet, Colors.green, () {
              Navigator.push(context, MaterialPageRoute(
                  builder: (_) => StudentFeeDashboard(studentId: student.id)));
            }),
          ),
          const SizedBox(width: 12),
...
// ════════════════════════════════════════════════════════════════
// CREDIT ACTIVITIES PAGE (placeholder — keep as-is)
// ════════════════════════════════════════════════════════════════
